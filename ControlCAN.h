...
{	
	...
	DWORD __stdcall VCI_Receive2(DWORD DeviceType,DWORD DeviceInd,DWORD CANInd,BYTE *Recvbuf);//功能同receive函数，未采用结构体。
	...
	DWORD __stdcall VCI_CANStopCallBack(DWORD DeviceIndex);
		...
		DWORD __stdcall VCI_ConnectDevice(DWORD DeviceType,DWORD DeviceInd);//查询设备是否在系统中，仅针对USB-CAN设备
